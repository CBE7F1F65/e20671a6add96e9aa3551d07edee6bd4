...
{
	image_t image;
	if(size!=texh*texw*bpb){
		nge_print("Fatal error,exist!\n");
		return 0;
	}
	image.data = (uint8*)mbuf;
	image.bpb = bpb;
	image.texw = texw;
	image.texh = texh;
	image.w = texw;
	image.h = texh;
	image.dtype = DISPLAY_PIXEL_FORMAT_8888;
	image.rcentrex = 0;
	image.rcentrey = 0;
	image.mode = GET_PSM_COLOR_MODE(image.dtype);
	image.mask = CreateColor(255,255,255,255,image.dtype);
	image_save_tga(&image,filename,1,1);
	return 1;
}
...
int buffer2image(image_p pimg,char* mbuf,int size,int texw,int texh,int bpb)
{
	...
	pimg->data = (uint8*)mbuf;
	pimg->bpb = bpb;
	pimg->texw = texw;
	pimg->texh = texh;
	pimg->w = texw;
	pimg->h = texh;
	pimg->dtype = DISPLAY_PIXEL_FORMAT_8888;
	pimg->rcentrex = 0;
	pimg->rcentrey = 0;
	pimg->mode = GET_PSM_COLOR_MODE(pimg->dtype);
	pimg->mask = CreateColor(255,255,255,255,pimg->dtype);
	pimg->texid = image_tid++;
	return 1;
}
