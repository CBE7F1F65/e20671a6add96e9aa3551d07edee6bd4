...
//			FrontDisplay::fdisp.SetState(FDISP_PANEL, 0);
	...
	/*
		...
		else if(tdepth == 2)
		{
			...
			if(!strcmp(savefilename, ""))
			{
				...
				}while(_access(Export::Resource_MakePath(savefilename), 00) != -1);
			}
		...
		else if(tdepth == 3)
		{
			...
			if(Export::Input_GetDIKey(KS_UP, DIKEY_DOWN) || Export::Input_GetDIKey(KS_DOWN, DIKEY_DOWN))
			{
				tnowchar = 0;
			}
			...
			if(Export::Input_GetDIKey(KS_ENTER, DIKEY_DOWN) || Export::Input_GetDIJoy(JS_FIRE, DIKEY_DOWN))
			{
			...
			if(Export::Input_GetDIKey(DIK_BACKSPACE, DIKEY_DOWN))
			{
				if(tnowchar)
				{
			...
			if(Export::Input_GetDIKey(DIK_DELETE, DIKEY_DOWN))
			{
				SE::push(SE_SYSTEM_CANCEL);
				if(!tsec)
					username[tnowchar] = ' ';
	...
	skip1:
			...
			if(tsec == 2)
			{
				if(Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN))
				{
					...
					SetCurrentDirectory(Export::Resource_MakePath(res.resdata.replayfoldername));
					char * buffer;
					char enumfile[M_STRMAX];
					strcpy(enumfile, res.resdata.replayfoldername);
					strcat(enumfile, "*.");
					strcat(enumfile, res.resdata.replayextensionname7);
					buffer = Export::Resource_EnumFiles(enumfile);
					while(buffer != NULL)
					{
						...
						if(!Export::Resource_EnumFiles())
							break;
					}
					SetCurrentDirectory(Export::Resource_MakePath(""));
				...
				else if(Export::Input_GetDIKey(KS_QUICK, DIKEY_DOWN))
				{
					SE::push(SE_SYSTEM_CANCEL);
					tdepth = 7;
				}
		...
		{
			InfoSelect::avoid = true;
			Export::Input_SetDIKey(KS_UP, false);
			Export::Input_SetDIKey(KS_DOWN, false);
		}
		else if(tdepth == 6)
		{
	//		rpy.Fill();
	...
	skip2:
			if((tsec == 1 || !tinsert) && Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN) ||
				tsec == 0 && Export::Input_GetDIJoy(JS_FIRE, DIKEY_DOWN))
			{
				DWORD sec;
				sec = data.sLinkType(DATAS_TOP);
				sec = data.sLinkDiff(sec, nowdifflv);
