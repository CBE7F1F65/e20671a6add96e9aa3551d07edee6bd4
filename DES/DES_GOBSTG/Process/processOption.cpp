...
int Process::processOption()
{
	...
	if(!tdepth)
	{
		if(Export::Input_GetDIKey(KS_QUICK, DIKEY_DOWN))
		{
			SE::push(SE_SYSTEM_CANCEL);
			selsys[tselsys].select = selsys[tselsys].nselect-1;
		}
		if((Export::Input_GetDIKey(KS_QUICK, DIKEY_DOWN) || Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN)) && tsel == selsys[tselsys].nselect-1)
		{
			SE::push(SE_SYSTEM_CANCEL);
		...
		if(Export::Input_HaveJoy() &&
			(Export::Input_GetDIKey(DIK_TAB, DIKEY_DOWN) ||
			!tsel && (Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN) || Export::Input_GetDIKey(KS_RIGHT, DIKEY_DOWN))))
		{
		...
		{
			if(Export::Input_GetDIKey(KS_LEFT, DIKEY_DOWN) ||
				Export::Input_GetDIKey(KS_RIGHT, DIKEY_DOWN) ||
				Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN))
			{
				SE::push(SE_SYSTEM_OK);
				screenmode = 1 - screenmode;
				Export::System_SetState(HGE_WINDOWED, !(bool)screenmode);
				Export::System_SetState(HGE_HIDEMOUSE, (bool)screenmode);
			}
		}
		else if(tsel == 2 || tsel == 3)
		{
			...
			{
				tvol = sevol;
				if(!Export::Channel_IsPlaying(SE::se[SE_BOSS_TIMEOUT].chn))
					SE::push(SE_BOSS_TIMEOUT);
			}

			if(Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN))
			{
				tvol += 5;
				if(tvol > 100)
					tvol = 0;
			}
			else if(Export::Input_GetDIKey(KS_LEFT, DIKEY_DOWN))
			{
			...
			else if(Export::Input_GetDIKey(KS_RIGHT, DIKEY_DOWN))
			{
				SE::push(SE_SYSTEM_SELECT);
				tvol += 5;
				if(tvol > 100)
		...
		else if(tsel == 4 && Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN))
		{
	...
	else
	{
		if (tsel != 5)
		{
			...
			{
				if(Export::Input_GetDIJoy(i, DIKEY_DOWN) &&
					!Export::Input_GetDIJoy(JOY_LEFT, DIKEY_DOWN) &&
					!Export::Input_GetDIJoy(JOY_RIGHT, DIKEY_DOWN) &&
					!Export::Input_GetDIJoy(JOY_UP, DIKEY_DOWN) &&
					!Export::Input_GetDIJoy(JOY_DOWN, DIKEY_DOWN))
				{
					SE::push(SE_SYSTEM_OK);
		...
		if(Export::Input_GetDIKey(KS_ENTER, DIKEY_DOWN) ||
			Export::Input_GetDIKey(DIK_TAB, DIKEY_DOWN) ||
			Export::Input_GetDIKey(KS_LEFT, DIKEY_DOWN) ||
			tsel == 5 && (Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN) || Export::Input_GetDIKey(KS_QUICK, DIKEY_DOWN)) ||
			Export::Input_GetDIKey(KS_QUICK, DIKEY_DOWN) && !Export::Input_GetDIJoy(JS_QUICK, DIKEY_DOWN) ||
			Export::Input_GetDIKey(KS_FIRE, DIKEY_DOWN) && !Export::Input_GetDIJoy(JS_FIRE, DIKEY_DOWN))
		{
			SE::push(SE_SYSTEM_OK);
			tdepth = 0;
			for(list<Selector>::iterator it=selsys[tselsys].sel.begin();it!=selsys[tselsys].sel.end();it++)
	...
	SE::vol = sevol;
	Export::Channel_SetVolume(channel, bgmvol);
