...
//		scr.SetIntValue(SCR_RESERVEBEGIN, 0xff);
	...
	*/
/*
	if(selsys[scr.GetIntValue(SCR_RESERVEBEGIN+1)].sel.size() && Export::Input_GetDIKey(KS_QUICK, DIKEY_UP))
	{
		SelectSystem::ClearAll();
...
//		state = STATE_START;
	...
	else if(scr.GetIntValue(SCR_RESERVEBEGIN) >= 0x100 && !BGLayer::ubg[UBGID_FGPAUSE].flag)
	{
		...
		if(state == STATE_START)
		{
			...
			Export::Input_SetDIKey(KS_LEFT_(0), (bool)(saveInput & 0x0100));
			Export::Input_SetDIKey(KS_RIGHT_(0), (bool)(saveInput & 0x0200));
			Export::Input_SetDIKey(KS_UP_(0), (bool)(saveInput & 0x0400));
			Export::Input_SetDIKey(KS_DOWN_(0), (bool)(saveInput & 0x0800));
			Export::Input_SetDIKey(KS_FIRE_(0), (bool)(saveInput & 0x1000));
			Export::Input_SetDIKey(KS_QUICK_(0), (bool)(saveInput & 0x2000));
			Export::Input_SetDIKey(KS_CHARGE_(0), (bool)(saveInput & 0x4000));
			Export::Input_SetDIKey(KS_SLOW_(0), (bool)(saveInput & 0x8000));

			Export::Input_SetDIKey(KS_LEFT_(1), (bool)(saveInput & 0x0001));
			Export::Input_SetDIKey(KS_RIGHT_(1), (bool)(saveInput & 0x0002));
			Export::Input_SetDIKey(KS_UP_(1), (bool)(saveInput & 0x0004));
			Export::Input_SetDIKey(KS_DOWN_(1), (bool)(saveInput & 0x0008));
			Export::Input_SetDIKey(KS_FIRE_(1), (bool)(saveInput & 0x0010));
			Export::Input_SetDIKey(KS_QUICK_(1), (bool)(saveInput & 0x0020));
			Export::Input_SetDIKey(KS_CHARGE_(1), (bool)(saveInput & 0x0040));
			Export::Input_SetDIKey(KS_SLOW_(1), (bool)(saveInput & 0x0080));

			if (musicID >= 0)
			{
				Export::Channel_SetVolume(channel, 0);
				Export::Channel_Resume(channel);
				musicSlide(1.5f, -1);
			}
