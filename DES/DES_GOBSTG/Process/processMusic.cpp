...
//		SE::push(SE_SYSTEM_OK);
	...
	else if(Export::Input_GetDIKey(KS_PAUSE, DIKEY_DOWN))
	{
		if(Export::Channel_IsPlaying(channel))
		{
			SE::push(SE_SYSTEM_OK);
			musicChange(musicID+1, true);
		}
	}
	else if(Export::Input_GetDIKey(KS_CHARGE, DIKEY_DOWN))
	{
		if(Export::Channel_IsPlaying(channel))
		{
			SE::push(SE_SYSTEM_OK);
	...
	else if (Export::Input_GetDIKey(KS_SLOW, DIKEY_DOWN))
	{
		if(Export::Channel_IsPlaying(channel))
		{
			SE::push(SE_SYSTEM_OK);
