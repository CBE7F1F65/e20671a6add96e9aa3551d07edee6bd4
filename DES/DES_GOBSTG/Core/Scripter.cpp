...
/*
	...
	if(!binmode)
	{
		...
		BYTE * _content;
		DWORD _size;
		Export::Resource_AttachPack(Data::data.scriptfilename, Data::data.password);
		_content = Export::Resource_Load(Data::data.scrbinname, &_size);
		...
		Export::Resource_Free(_content);

		for (int i=0; i<M_SCRIPTFOLDERMAX; i++)
		{
			if (strlen(BResource::res.resdata.scriptfoldername[i]))
			{
				SetCurrentDirectory(Export::Resource_MakePath(BResource::res.resdata.scriptfoldername[i]));
		...
		Export::Resource_CreatePack(Data::data.scriptfilename, Data::data.password, &memfile, NULL);
	...
	else
	{
		...
		Export::Resource_AttachPack(Data::data.scriptfilename, Data::data.password);
		bincontent = Export::Resource_Load(Data::data.scrbinname, &binsize);
		if(bincontent)
		{
...
#endif
			Export::Resource_Free(bincontent);
			bincontent = NULL;
			return false;
		}
		Export::Resource_Free(bincontent);
