...
/*
		...
		HANDLE hFile = CreateFile(Export::Resource_MakePath(filename), GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
		{
...
/*
		...
		HANDLE hFile = CreateFile(Export::Resource_MakePath(ofilename), GENERIC_WRITE, FILE_SHARE_READ, NULL, CREATE_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL);
		if (hFile == INVALID_HANDLE_VALUE)
		{
	...
	* /
		...
		double sum = 0;
		double sumsub = 0;
		while (rpyframe[index].input != 0xffff)
		{
