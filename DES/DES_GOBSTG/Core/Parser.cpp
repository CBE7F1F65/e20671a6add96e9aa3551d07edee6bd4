		...
		for(vector<Script>::iterator i=pnow->begin();i!=pnow->end();i++)
		{
			...
			case SCRKW_ACTION:
				...
				case SCR_SETKEY:
					if (rv = Copy(&i, 2))
					{
						Export::Input_SetDIKey(CAST(d[0]), CAST(d[1]));
					}
					break;
				case SCR_DISABLEALLKEY:
					if (rv = true)
					{
						Export::Input_SetDIKey(KS_FIRE_MP, false);
						Export::Input_SetDIKey(KS_QUICK_MP, false);
						Export::Input_SetDIKey(KS_CHARGE_MP, false);
						Export::Input_SetDIKey(KS_SLOW_MP, false);
						Export::Input_SetDIKey(KS_UP_MP, false);
						Export::Input_SetDIKey(KS_DOWN_MP, false);
						Export::Input_SetDIKey(KS_LEFT_MP, false);
						Export::Input_SetDIKey(KS_RIGHT_MP, false);
					}
					break;
			...
			case SCRKW_BASIC:
				...
				case SCRKW_CLASS_LAYER:
					...
					case SCR_BGVALUE:
						...
						{
							int _tdi = CAST(d[0]);
							BGLayer::ubg[_tdi].valueSet(mp.tex, CAST(d[1]), CAST(d[2]), CAST(d[3]), CAST(d[4]), CAST(d[5]), UCAST(d[6]));
						}
						break;
					case SCR_BGVALEX:
						if (rv = Copy(&i, 16))
						{
							int _tdi = CAST(d[0]);
							BGLayer::ubg[_tdi].valueSet(mp.tex, CAST(d[1]), CAST(d[2]), CAST(d[3]), CAST(d[4]), CAST(d[5]), CAST(d[6]), CAST(d[7]), CAST(d[8]), CAST(d[9]), CAST(d[10]), CAST(d[11]), CAST(d[12]), (bool)(CAST(d[13])), (bool)(CAST(d[14])), UCAST(d[15]));
						}
						break;
					case SCR_BGTEXRECT:
						if(rv = Copy(&i, 5))
